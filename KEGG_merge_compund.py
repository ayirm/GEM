...
"""
Tam Biyoinformatik Pipeline: FASTQ -> Alignment -> Anotasyon -> KEGG -> Reaksiyon -> Bileşik
"""
            ...
            try:
                ...
                while not job_finished:
                    ...
                    if status_response.status_code == 404:
                        sys.stdout.write(f"Polling job status... (job not ready, waiting) {'1-2'[spinner_idx % 4]}")
                        sys.stdout.flush()
                        spinner_idx += 1
                        time.sleep(5)
                        continue
                    ...
                    if "results" in status_data or status_data.get("jobStatus") == "FINISHED":
                        ...
                        print(f"\nJob has an unexpected status: {status_data.get('jobStatus')}")
...
# -----------------------------------------------------------------
# YENİ KEGGCLIENT SINIFI (GELİŞTİRİLDİ)
# -----------------------------------------------------------------

class KEGGClient:
    """
    KEGG veritabanıyla web servisi (API) üzerinden iletişim kurmak
    için kullanılan istemci sınıfı. (requests tabanlı)
    """
    BASE_URL = "https://rest.kegg.jp"
    
    def __init__(self, rate_limit_sec=0.35):
        """Sınıf başlatıldığında bir mesaj yazdırır."""
        print("[KEGGClient] İstemci başlatıldı. (Rate limit: {rate_limit_sec}sn)")
    ...
    def get_linked_entries(self, target_db, source_id):
        ...
        örn: /link/compound/rn:R00200
        """
        endpoint = f"/link/{target_db}/{source_id}"
        # print(f"[KEGGClient Link Sorgusu] '{source_id}' -> '{target_db}'...") # Çok fazla çıktı üretiyor, kapalı
    ...
    def get_entry_details(self, entry_id):
        ...
        (YENİ EKLENDİ)
        """
        endpoint = f"/get/{entry_id}"
        # print(f"[KEGGClient Get Sorgusu] '{entry_id}' detayları...") # Çok fazla çıktı üretiyor, kapalı
        raw_data = self._make_request(endpoint)
        return raw_data

# -----------------------------------------------------------------
# YENİ YARDIMCI PARSER FONKSİYONU
# -----------------------------------------------------------------

def parse_kegg_name(entry_text):
    """
    KEGG 'get' çıktısından 'NAME' alanını ayrıştırır.
    (YENİ EKLENDİ)
    """
    if not entry_text:
        return "N/A"
...
def run_alignement(ref_path:str, fastq1_loc:str, fastq2_loc:str, out_loc:str, threads=4):
    ...
    if not run_cmd(f"bowtie2-build --threads {str(threads)} {ref_path} {ref_index_prefix}","Indexing Reference File"):
        return None # Hata durumunda None dön
    if not run_cmd(f"bowtie2 -x {ref_index_prefix} -1 {fastq1_loc} -2 {fastq2_loc} -S {sam_file}","Aligning FastQ reads"):
        return None # Hata durumunda None dön
    return sam_file
...
def run_samtools(sam_path:str, threads:int, out_loc:str):
    ...
    if not run_cmd(f"samtools view -uS -o {bam_loc} {sam_path}","SAM to BAM conversion"): return None
    if not run_cmd(f"samtools sort -@ {str(threads)} -T {tmp_file} -o {sorted_loc} {bam_loc}", "BAM file sorting"): return None
    if not run_cmd(f"samtools index {sorted_loc}","İndexing the sorted BAM file"): return None
    return sorted_loc

def run_bcftools(ref_path:str, sorted_path:str ,out_loc:str):
    """Collapses the reads and writes variant information to a VCF file."""
    os.makedirs(out_loc, exist_ok=True)
    ...
    if not run_cmd(f"bcftools mpileup -f {ref_path} {sorted_path} | bcftools call -mv -Ob -o {bcf_loc}", "Collapsing reads to a BCF file"): return None
    if not run_cmd(f"bcftools convert -O v -o {vcf_loc} {bcf_loc}","Converting BCF to VCF"): return None
    if not run_cmd(f"bgzip -c {vcf_loc} > {gz_loc}", "Compressing VCF"): return None
    if not run_cmd(f"tabix -p vcf {gz_loc}", "Indexing compressed VCF"): return None
    return gz_loc
...
def run_reference_assembly(ref_path:str, gz_loc:str, out_loc:str):
    ...
    if not run_cmd(f"bcftools consensus -f {ref_path} {gz_loc} > {cons_loc}","Creating consensus sequence"): return None
    return cons_loc

def run_annotation(cons_path:str, out_loc:str,prkName="prokkaAnnotes"):
    """Runs prokka to annote the reference assembled genome"""
    os.makedirs(out_loc, exist_ok=True)
    if not run_cmd(f"prokka --outdir {out_loc} --prefix {prkName} --force {cons_path}","Annotation with Prokka"): return None, None
    return prkName, out_loc

def uniprot_search(ann_path:str, prkName="prokkaAnnotes", cache_file="go_cache.json"):
    """Uses prokka results to get GO terms via UniProt"""
...
# -----------------------------------------------------------------
# GÜNCELLENMİŞ KEGG_SEARCH FONKSİYONU (BİLEŞİK EKLENDİ)
# -----------------------------------------------------------------

def kegg_search(cds_values):
    """
    KEGGClient sınıfını kullanarak UniProt ID'lerini zenginleştirir.
    Akış: Gene -> Pathway -> Reaction -> Compound
    """
    ...
    print(f"\n--- Running KEGG Pathway, Reaction, and Compound Search ---")
    ...
    print(f"Processing {total_entries} entries for KEGG pathways, reactions, and compounds...")
    
    # --- YENİ (BİLEŞİK ÖNBELLEĞİ) ---
    # Bileşik isimlerini tekrar tekrar çekmemek için basit bir önbellek
    compound_name_cache = {}
    
    all_pathway_reactions = {} # {path_id: [rxn1, rxn2], ...}
    all_reaction_compounds = {} # {rxn_id: {cpd_id: cpd_name, ...}, ...}
    ...
    for i, entry in enumerate(cds_values, 1):
        ...
        entry["pathways"] = "N/A"
        entry["reactions_by_pathway"] = "N/A"
        entry["compounds_by_reaction"] = "N/A" # YENİ ALAN
        ...
        try:
            ...
            # --- Akış 2: Pathway -> Reaksiyon ---
            pathway_reaksiyon_iliskisi = {}
            tum_reaksiyonlar_bu_gen_icin = set()
            
            for p_id in pathway_idler:
                # `all_pathway_reactions` önbelleğini kontrol et
                if p_id not in all_pathway_reactions:
                    ...
                    all_pathway_reactions[p_id] = sorted(list(set(reaksiyonlar)))
                
                # Önbellekten (veya yeni çekilen veriden) al
                reaksiyonlar_bu_pathwayde = all_pathway_reactions[p_id]
                
                if reaksiyonlar_bu_pathwayde:
                    pathway_reaksiyon_iliskisi[p_id] = reaksiyonlar_bu_pathwayde
                    tum_reaksiyonlar_bu_gen_icin.update(reaksiyonlar_bu_pathwayde)
            
            if not pathway_reaksiyon_iliskisi:
            ...
            # --- Akış 3: Reaksiyon -> Bileşik (YENİ EKLENDİ) ---
            reaksiyon_bilesik_iliskisi = {}
            
            for rxn_id in tum_reaksiyonlar_bu_gen_icin:
                # `all_reaction_compounds` önbelleğini kontrol et
                if rxn_id not in all_reaction_compounds:
                    # Bu reaksiyon için bileşikleri ve isimlerini bul
                    bilesikler_bu_reaksiyonda = {}
                    compound_idler = kegg_api_client.get_linked_entries("compound", rxn_id)
                    
                    for cpd_id in compound_idler:
                        # `compound_name_cache` önbelleğini kontrol et
                        if cpd_id not in compound_name_cache:
                            detay_metni = kegg_api_client.get_entry_details(cpd_id)
                            compound_name_cache[cpd_id] = parse_kegg_name(detay_metni)
                        
                        # Önbellekten (veya yeni çekilen veriden) ismi al
                        bilesik_adi = compound_name_cache[cpd_id]
                        bilesikler_bu_reaksiyonda[cpd_id] = bilesik_adi
                    
                    # Ana reaksiyon önbelleğine kaydet
                    all_reaction_compounds[rxn_id] = bilesikler_bu_reaksiyonda
                
                # Bu genin sonucuna ekle
                if all_reaction_compounds[rxn_id]:
                    reaksiyon_bilesik_iliskisi[rxn_id] = all_reaction_compounds[rxn_id]

            if reaksiyon_bilesik_iliskisi:
                entry["compounds_by_reaction"] = json.dumps(reaksiyon_bilesik_iliskisi)
            else:
                entry["compounds_by_reaction"] = "No compounds found for these reactions"

        except Exception as e:
            print(f"\nError processing KEGG workflow for {kegg_id}: {e}", file=sys.stderr)
            entry["pathways"] = f"Error: {e}"
            entry["reactions_by_pathway"] = f"Error: {e}"
            entry["compounds_by_reaction"] = f"Error: {e}"
...
def run_pipeline(ref_path, fastq1, fastq2, out_dir="results", threads=4, prk_prefix="prokkaAnnotes", excel_file="annotation_results.xlsx"):
    ...
    align_and_sam = os.path.join(out_dir, "sam_bam")
    bcf_and_gz = os.path.join(out_dir, "vcf_consensus")
    ...
    if not check_existing_file(sam_file, "Alignemnt and Sam file creation"):
        ...
        if not sam_file: print("Alignment failed, exiting."); sys.exit(1)
    ...
    if not check_existing_file(sorted_bam, "Sorting the Bam file"):
        ...
        if not sorted_bam: print("Samtools failed, exiting."); sys.exit(1)

    if not check_existing_file(gz_vcf, "Collapsing Reads"):
        gz_vcf = run_bcftools(ref_path, sorted_bam, out_loc=bcf_and_gz)
        if not gz_vcf: print("BCFtools mpileup/call failed, exiting."); sys.exit(1)
    ...
    if not check_existing_file(cons_fasta, "Consensus File Creation"):
        ...
        if not cons_fasta: print("BCFtools consensus failed, exiting."); sys.exit(1)
    ...
    if not (check_existing_file(prk_gbf, "annotation (Prokka)") or check_existing_file(prk_gbk, "annotation (Prokka)")):
        ...
        if not prkName: print("Prokka annotation failed, exiting."); sys.exit(1)
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Full genome-to-pathways-to-compounds pipeline")
